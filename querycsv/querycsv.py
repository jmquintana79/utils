...
def import_csv(db, filename, table_name=None, overwrite=False):
    ...
    with as_connection(db) as conn:
        ...
        reader = csv.reader(open(filename, newline=None), dialect)
        #reader = csv.reader(open(filename, 'rU'), dialect) ## deprecated version of open “U” mode
        column_names = next(reader)
        colstr = ",".join('[{0}]'.format(col) for col in column_names)
        conn.execute('drop table if exists %s;' % table_name)
        conn.execute('create table %s (%s);' % (table_name, colstr))
        for row in reader:
            #vals = [str(cell.encode('utf-8')) for cell in row] ## it is not working WHERE statement for SQL queries
